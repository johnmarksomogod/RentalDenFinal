...
export default function EditBookingModal({
  ...
  styles,
  formatDate,
  // CalendarModalComponent no longer needed — BookingForm has built-in calendar
  CalendarModalComponent,
  // Optional extras from BookingsScreen
  onAssignDriver,
  extraContent,
}) {
  ...
  // ── Initialise editable booking from prop ─────────────────────────────────
  useEffect(() => {
    if (booking && typeof booking === "object") {
      setEditableBooking({
        ...
        // Delivery — always normalise so BookingForm never sees undefined
        delivery_option:      booking.delivery_option === "deliver" ? "deliver" : "pickup",
        ...
        assigned_driver_email:booking.assigned_driver_email || "",
  ...
  // Delivery option — normalised boolean helper used by the status bar
  const isDelivery = editableBooking?.delivery_option === "deliver";

  const shouldShowDeleteButton = useMemo(
    () => editableBooking?.status === "cancelled",
    [editableBooking?.status]
  );

  // ── Status change (quick action — no full form save needed) ───────────────
  const handleStatusChange = useCallback(async (newStatus) => {
    if (!editableBooking?.id) return;

    if (newStatus === "declined" && !declineReason.trim()) {
      // Keep sheet open so user can fill in the reason
      return;
    }
    ...
    try {
      ...
      setEditableBooking((prev) =>
        prev
          ? { ...prev, status: newStatus, decline_reason: statusUpdate.decline_reason }
          : null
      );
      setStatusSheetVisible(false);
      setDeclineReason("");
  ...
  /**
   * Status bar — always visible at the top of the modal so owners can
   * change status without scrolling. It also shows the Assign Driver
   * button for delivery bookings.
   */
    ...
    return (
      <View style={ebS.statusBar}>
        ...
        <View style={[ebS.currentStatusBadge, { backgroundColor: statusColor + "22" }]}>
          ...
          <Text style={[ebS.currentStatusTxt, { color: statusColor }]}>
        ...
        {/* Assign/Reassign driver — only for delivery bookings */}
        {onAssignDriver && isDelivery && (
          ...
          >
            <Ionicons
              name={editableBooking.assigned_driver ? "swap-horizontal" : "person-add-outline"}
              size={14}
              color="#374151"
            />
            <Text style={ebS.changeStatusTxt}>
              {editableBooking.assigned_driver ? "Reassign" : "Assign Driver"}
            </Text>
          </TouchableOpacity>
        )}
        ...
        {isDelivery && (
          <View style={ebS.deliveryBadge}>
            <Ionicons name="car-outline" size={12} color="#2563eb" />
            <Text style={ebS.deliveryBadgeTxt}>Delivery</Text>
          </View>
        )}
      </View>
    );
  ...
  /**
   * Status-change bottom sheet — supports the normal status flow plus
   * an admin-override row of all statuses.
   */
        ...
        <View style={ebS.sheet}>
          ...
          <ScrollView style={ebS.sheetBody} showsVerticalScrollIndicator={false}>
                ...
                <View key={status}>
                  <TouchableOpacity
                    style={[
                      ebS.option,
                      {
                        borderColor:     STATUS_COLOR[status] + "44",
                        backgroundColor: STATUS_COLOR[status] + "0d",
                      },
                    ]}
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={[ebS.optionLabel, { color: STATUS_COLOR[status] }]}>
                  ...
                  {status === "declined" && (
                    <View style={ebS.declineInput}>
                      ...
                      <TextInput
                        style={[
                          ebS.declineBox,
                          !declineReason.trim() && ebS.declineBoxEmpty,
                        ]}
                        value={declineReason}
                      ...
                      />
                      {!declineReason.trim() && (
                        <Text style={ebS.declineHint}>
                          Fill in reason, then tap "❌ Decline Booking" above.
                        </Text>
                      )}
                    </View>
                  )}
            ...
            <ScrollView
              ...
              contentContainerStyle={ebS.overrideRow}
            >
        ...
        >
          ...
          {!booking ? (
            <View style={{ flex: 1 }}>
              ...
              <View style={{ flex: 1, justifyContent: "center", alignItems: "center", padding: 20 }}>
                ...
                <TouchableOpacity
                  ...
                  style={{ marginTop: 20, backgroundColor: "#111827", paddingHorizontal: 20, paddingVertical: 10, borderRadius: 8 }}
                >
            ...
            <>
              ...
              {/* ── Status bar — always visible, no scroll needed ──────────── */}
              {renderStatusBar()}
              ...
              <ScrollView
                ...
                bounces={false}
              >
                {/* Extra content (e.g. DriverStatusTimeline) from BookingsScreen */}
                {extraContent && (
                  <View style={{ marginBottom: 12 }}>{extraContent}</View>
                )}

                {/* Decline reason — read-only if already declined */}
                {currentStatus === "declined" && editableBooking.decline_reason ? (
                  <View style={[styles.section, { marginBottom: 12 }]}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 8 }}>
                      <Ionicons name="close-circle-outline" size={16} color="#ef4444" />
                      <Text style={[styles.sectionTitle || { fontSize: 15, fontWeight: "700" }, { color: "#ef4444" }]}>
                        Decline Reason
                      </Text>
                    </View>
                    <Text style={{ fontSize: 14, color: "#374151", lineHeight: 20 }}>
                      {editableBooking.decline_reason}
                    </Text>
                  </View>
                ) : null}

                {/*
                 * BookingForm owns all field rendering, including:
                 *  - Customer info
                 *  - Vehicle/variant pickers (Dropdown + internal data fetching)
                 *  - Built-in CalendarModal (no external component needed)
                 *  - Rental period + price summary
                 *  - Pickup / Delivery toggle (always saves delivery_option)
                 *  - Gov-ID upload
                 *  - Booking status dropdown (isEdit=true)
...
const ebS = StyleSheet.create({
  ...
  deliveryBadgeTxt: { fontSize: 11, fontWeight: "700", color: "#2563eb" },
